...
def skewt(request):
    ...
    fig, (ax, ax_wind) = plt.subplots(1, 2, figsize=(10, 10),
    ...
    for T0 in range(-40, 100, 10):
        ...
        ax.plot(skew_temp(T_dry, P_line), P_line,
                color='#2d6a4f', linewidth=0.5, alpha=0.5)
    ...
    for T in range(-80, 50, 10):
        ...
        ax.text(skew_temp(T, 1013), 1020, f'{T}°',
    ...
    if wind_pressures:
        # thin out barbs so they don't overlap — keep ~20 levels
        wp = np.array(wind_pressures)
        wu = np.array(wind_u)
        wv = np.array(wind_v)

        # sample evenly in log-pressure space
        indices = np.unique(
        ...
        ax_wind.barbs(
            np.zeros_like(wp),   # all barbs on the centre line
            wp,
            wu,
            wv,
            length=6,
            linewidth=0.8,
            color='#f4d35e',     # warm yellow — same accent feel as the rest
            zorder=3
        )
...
def emagram(request):
    ...
    # -------------------------
    # ISOTHERMS (TRUE EMAGRAM)
    # vertical lines
    # -------------------------
    P_line = np.linspace(1050, 100, 100)
    for T in range(-80, 50, 10):
        ax.plot(
            np.full_like(P_line, T),
            P_line,
            color='#444',
            linewidth=0.6,
            alpha=0.5
    ...
    # -------------------------
    # DRY ADIABATS (correct emagram form)
    # θ = T * (P0/P)^k
    # -------------------------
    P0 = 1013.25
    k = 0.286

    for theta in range(220, 380, 10):  # potential temperature range
        P = np.linspace(1050, 100, 120)
        T = theta * (P / P0) ** k - 273.15
    ...
    ax.plot(dewpoints, pressures, color='#457b9d', linewidth=2, label='Dewpoint')
...
def tephigram(request):
    ...
    pressures, temps, dewpoints = [], [], []
    for t in telemetry:
        p = estimate_pressure(t['alt'], t['temp'], t['humidity'])   # or use real pressure if available
        dp = dewpoint(t['temp'], t['humidity'])
        pressures.append(p)
        temps.append(t['temp'])           # ← should already be in Celsius
        dewpoints.append(dp)
    ...
    # === CRITICAL FIX ===
    ax.set_xlim(-80, 40)                    # ← This was the main bug
    # ====================

    # Isotherms (slanted for Tephigram)
    ...
    # Dry Adiabats (straight lines on Tephigram)
    P0 = 1013.25
    k = 0.2854
    for theta_k in range(230, 400, 10):          # potential temperature in K
        P = np.linspace(1050, 100, 150)
        T = theta_k * (P / P0) ** k - 273.15
        ax.plot(T, P, color='#2d6a4f', linewidth=0.7, alpha=0.5)

    # Plot sounding
    ax.plot(temps, pressures, color='#e63946', linewidth=2.5, label='Temperature')
    ax.plot(dewpoints, pressures, color='#457b9d', linewidth=2.5, label='Dewpoint')
...
def stuve(request):
    ...
    for theta in range(220, 380, 10):
        P = np.linspace(1050, 100, 120)
        T = theta * (P / P0) ** k - 273.15
        ax.plot(T, P, color='#2d6a4f', alpha=0.4, linewidth=0.5)
...
def equiv_potential_temperature(temp_c, dewpoint_c, pressure_hpa):
    """Equivalent potential temperature θe (°C) — Bolton 1980 approx."""
    Tk  = temp_c + 273.15
    Td  = dewpoint_c + 273.15
    es  = 6.112 * math.exp(17.67 * dewpoint_c / (dewpoint_c + 243.5))
    ws  = 0.622 * es / (pressure_hpa - es)
    Lv  = 2.5e6
    cp  = 1004.0
    return Tk * math.pow(1013.25 / pressure_hpa, 0.286) * math.exp(Lv * ws / (cp * Tk)) - 273.15
...
def compute_cape_cin(levels):
    """
    Simple parcel-theory CAPE & CIN.
    levels: list of dicts sorted surface→top with keys pressure, temp, dewpoint.
    ...
    es_sfc   = 6.112 * math.exp(17.67 * sfc['dewpoint'] / (sfc['dewpoint'] + 243.5))
    ws_sfc   = 0.622 * es_sfc / (p_sfc - es_sfc)

    # LCL
    T_lcl    = 1.0 / (1.0 / (Td_sfc - 56) + math.log(T_sfc / Td_sfc) / 800.0) + 56
    p_lcl    = p_sfc * math.pow(T_lcl / T_sfc, 3.4965)

    Lv = 2.5e6; Rd = 287.05; cp = 1004.0

    cape = 0.0
    cin  = 0.0
    T_parcel = T_sfc  # parcel temp in K
    ...
    for i in range(1, len(levels)):
        ...
        if p_mid > p_lcl:
            ...
            # moist adiabatic (iterative)
            for _ in range(3):
                Tp   = T_parcel
                es_p = 6.112 * math.exp(17.67 * (Tp - 273.15) / ((Tp - 273.15) + 243.5))
                ws_p = 0.622 * es_p / (p_mid - es_p)
                num  = Rd * Tp + Lv * ws_p
                den  = cp + Lv * Lv * ws_p * 0.622 / (Rd * Tp * Tp)
                T_parcel -= (Rd * Tp / p1) * (num / den) / cp * dp

        T_env  = curr['temp'] + 273.15
        dT     = T_parcel - T_env  # buoyancy
        dz     = (Rd * T_env / (9.80665 * p_mid)) * dp * 100  # layer depth m
    ...
    return round(cape, 1), round(min(cin, 0.0), 1)


def compute_indices(levels):
    """
    Compute common sounding indices.
    ...
    sfc  = levels[0]
    T850 = next((l for l in levels if abs(l['pressure'] - 850) < 20), None)
    T700 = next((l for l in levels if abs(l['pressure'] - 700) < 20), None)
    T500 = next((l for l in levels if abs(l['pressure'] - 500) < 20), None)
    ...
    for i in range(1, len(levels)):
        ...
        T_av = (levels[i - 1]['temp'] + levels[i]['temp']) / 2.0 + 273.15
        es   = 6.112 * math.exp(17.67 * (T_av - 273.15) / ((T_av - 273.15) + 243.5))
        e    = (rh / 100.0) * es
        w    = 0.622 * e / (levels[i]['pressure'] - e)
        pw  += w * dp * 100 / 9.80665  # mm
    pw = round(pw, 1)

    # LCL
    lcl_p = round(lcl_pressure(sfc['temp'], sfc['dewpoint'], sfc['pressure']), 1)

    # Tropopause — first level where temp starts increasing after lapse below 300 hPa
    trop_p = None
    trop_alt = None
    ...
    # Wind shear
    def uv(d):
        if d['vel_h'] is None or d['heading'] is None:
            return None, None
    ...
    # Max wind
    wind_levels = [l for l in levels if l['vel_h'] is not None]
    max_wind     = max(wind_levels, key=lambda l: l['vel_h']) if wind_levels else None
...
def sonde_atmosphere_json(request):
    ...
    Returns per-level data with estimated pressure, dewpoint,
    ...
    for t in qs:
        ...
        levels.append({
            ...
            'pressure_measured':   t['pressure'],   # raw sensor, may be None
            'temp':                t['temp'],
            'dewpoint':            round(dp, 2),
            'humidity':            t['humidity'],
    ...
    # sort surface → top for index computation
    levels_sorted = sorted(levels, key=lambda l: -l['pressure_estimated'])

    # build index-friendly list
    idx_input = [{
